...
var CONFIG_DEFAULTS = {
  ...
  // 日時の基準タイムゾーン。
  // Apps Script プロジェクトのタイムゾーン設定が意図と違っていても（作成時の既定が
  // America/New_York になることがある）、ここの値で動くようにしている。
  // トリガーの実行時刻、フォルダ名・台帳の日時、年齢計算の基準日すべてに効く。
  TIMEZONE: 'Asia/Tokyo',

  // 定期実行の時刻。カンマ区切りで複数指定でき、installTrigger() が時刻ごとに
  // 1日1回のトリガーを登録する。TIMEZONE 基準。
  //
  // Apps Script は分単位の正確な実行を保証せず、指定時刻の前後15分ほどの幅が出る。
  // メールが無ければ何もせず終わるので、時刻を増やしてもAPIコストは増えない。
  TRIGGER_TIMES: '8:30,17:00',
